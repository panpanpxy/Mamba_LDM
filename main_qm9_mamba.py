...
from qm9.models_mamba import get_optim, get_model, get_autoencoder, get_latent_diffusion
from equivariant_diffusion import en_mamba_diffusion
...
try:
    ...
    print(f"Failed to save file to wandb:{e}")
...
#如果有condition context维度设置及每个batch的mean，用compute_mean_mad函数
...
def main():
    ...
    if args.ema_decay > 0:
        #parameter指数移动平均值的模型副本---EMA模型
        model_ema = copy.deepcopy(model)
        #parameter指数平移对象，用于对模型参数的指数平移平均操作
    ...
    for epoch in range(args.start_epoch, args.n_epochs):
        ...
        if epoch % args.test_epochs == 0:
            if isinstance(model, en_mamba_diffusion.EnVariationalDiffusion):
