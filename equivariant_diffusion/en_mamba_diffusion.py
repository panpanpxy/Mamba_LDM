...
import math
import torch
from egnn import model_mamba
from torch.nn import functional as F
from equivariant_diffusion import utils as diffusion_utils


# Defining some useful util functions.
from equivariant_diffusion.en_diffusion import disabled_train
...
def clip_noise_schedule(alphas2, clip_value=0.001):
    """
    nosie schedule--描述noise在training&generate 中如何随t变化
    For a noise schedule given by alpha^2, this clips alpha_t / alpha_t-1. This may help improve stability during
    sampling.
    alpha^2描述了在diffusion/generative process中的noise schedule,控制每个t的噪声强度和信号权重
    """
    alphas2 = np.concatenate([np.ones(1), alphas2], axis=0)
    # 计算t和t-1之间alphas的比率，表示噪声增加的速率
    alphas_step = (alphas2[1:] / alphas2[:-1])
    # clip 范围[clip_value,1]---stablize data value when sampling
    alphas_step = np.clip(alphas_step, a_min=clip_value, a_max=1.)
    # np.cumprod累积乘积重新计算alphas2
...
def polynomial_schedule(timesteps: int, s=1e-4, power=3.):
    ...
    compared to linear nosie schedule(alphas2=1-beta) more flexible to control alphas_step
    power--控制噪声增加的速率，常见=3
    """
    steps = timesteps + 1
...
def cosine_beta_schedule(timesteps, s=0.008, raise_to_power: float = 1):
    ...
    Nosie 随着时间的变化呈现周期性的变化 平滑控制nosie的变化
    """
    steps = timesteps + 2
    x = np.linspace(0, steps, steps)
...
def gaussian_KL(q_mu, q_sigma, p_mu, p_sigma, node_mask):
    """Computes the KL distance between two normal distributions.
       for invariant feature(calculate kl_h)
        Args:
...
def gaussian_KL_for_dimension(q_mu, q_sigma, p_mu, p_sigma, d):
    """Computes the KL distance between two normal distributions.
       for variant feature(calculate kl_x)
        Args:
            q_mu: Mean of distribution q.
            q_sigma: Standard deviation of distribution q.
            p_mu: Mean of distribution p.
...
class PositiveLinear(torch.nn.Module):
    ...
    def forward(self, input):
        """softplus--确保weight output为正数
           F.linear--标准的线性变换 output=input*w+b
        """
...
class GammaNetwork(torch.nn.Module):
    """The gamma network models a monotonic increasing function. Construction as in the VDM paper.
       目标是生成‘gamma schedule'的parameter，用于控制dm中t的variation of noise -随着t的增加而平滑递增
    """
...
class EnVariationalDiffusion(torch.nn.Module):
    ...
    diffsuion process
    generative process-sample_p_xh_given_z0
    compute_loss
    """
    def __init__(
            self,
            dynamics: model_mamba.EGMN_dynamics_QM9, in_node_nf: int, n_dims: int,
            timesteps: int = 1000, parametrization='eps', noise_schedule='learned',
            noise_precision=1e-4, loss_type='vlb', norm_values=(1., 1., 1.),
            norm_biases=(None, 0., 0.), include_charges=True):
        super().__init__()

        assert loss_type in {'vlb', 'l2'} #define loss function type-- 1. vlb(变分下界) 2.l2(均方误差)
        ...
        assert parametrization == 'eps'#noising network eps
    ...
    def sigma(self, gamma, target_tensor):
        """Computes sigma given gamma.
           sigma-决定了nosie的幅度，控制每个t的nosie强度
        """
        return self.inflate_batch_array(torch.sqrt(torch.sigmoid(gamma)), target_tensor)

    def alpha(self, gamma, target_tensor):
        """Computes alpha given gamma.
           alpha--决定了SNR，影响数据恢复的过程
        """
        return self.inflate_batch_array(torch.sqrt(torch.sigmoid(-gamma)), target_tensor)
        # SNR--信噪比，衡量信号与噪声的比率
    ...
    def compute_error(self, net_out, gamma_t, eps):
        """Computes error,
        i.e. the most likely prediction of x.
        预测噪声和真实噪声之间的差异
        """
        eps_t = net_out
    ...
    def sample_p_xh_given_z0(self, z0, node_mask, edge_mask, context, fix_noise=False):
        """Samples x ~ p(x|z0).--z0 is decoded to x,h"""
        zeros = torch.zeros(size=(z0.size(0), 1), device=z0.device)  # 生成一个与z0相同批次大小的零tensor，用作初始化
        gamma_0 = self.gamma(zeros)  # 确定初始nosie的大小
    ...
    def sample_normal(self, mu, sigma, node_mask, fix_noise=False):
        """Samples from a Normal distribution.
           从正态分布N(mu_x,sigma2_x)中sample---return xh
        """
    ...
    def log_pxh_given_z0_without_constants(
        ...
        """calculate logP(x,h|z0) without constant(常数项)
           epsilon--防止数值为0的小常数，通常用于避免对数计算中log(0)的问题
        """
        ...
        # 计算整数特征(h_integer)的log likelihood leverage 累积分布函数(CDF)calculate高斯分布的likelihood
        ...
        # logP(x,h|z)=logP(x|z)+logP(h|z)
        #           =logP(x|z)+logP(h_integer)+logP(h_cat)
    ...
    def compute_loss(self, x, h, node_mask, edge_mask, context, t0_always):
        """Computes an estimator for the variational lower bound, or the simple loss (MSE).
           用于评估模型生成数据的质量以及真实分布的匹配程度
           根据t来计算模型总的loss =kl_prior+neg_log_constants+loss_t*num_terms+loss_t_0(conditional)
           t0_always:是否始终包含t=0的loss 如果self.training==True,t0_always=True;否则FLASE
        """
        ...
        # Normalize t to [0, 1]. Note that the negative t&s归一化到区间[0,1]
        ...
        # Sample zt ~ Normal(alpha_t x, sigma_t)--get eps(nosie)
        eps = self.sample_combined_position_feature_noise(
            n_samples=x.size(0), n_nodes=x.size(1), node_mask=node_mask)

        # Concatenate x, h[integer] and h[categorical].
        xh = torch.cat([x, h['categorical'], h['integer']], dim=2)
        # Sample z_t given x, h for timestep t, from q(z_t | x, h) --add nosie eps
        z_t = alpha_t * xh + sigma_t * eps

        diffusion_utils.assert_mean_zero_with_mask(z_t[:, :, :self.n_dims], node_mask)

        # Neural net prediction.as nosie predictor
        net_out = self.phi(z_t, t, node_mask, edge_mask, context)

        # Compute the error.  calculate the loss(l2/mes)between net_out(predict) and eps(ground truth)
        error = self.compute_error(net_out, gamma_t, eps)
        # Compute SNR_weight--used to 加权 loss_t
        if self.training and self.loss_type == 'l2':
        ...
        # loss中的常数项--calculate neg_log_constants(z_0)
        ...
        # Combining the terms
        if t0_always:  # t=0的loss需加上总Loss中 Loss=kl_prior+ neg_log_constants+loss_t*num_terms(estimator_loss_terms)+loss_t_0
    ...
    def forward(self, x, h, node_mask=None, edge_mask=None, context=None):
        ...
        # delta_log_px 对数体积变化量
        # stablize data ,便于计算vlb
        ...
        if self.training:
            # Only one forward pass when t0_always is False.(training mode)
            loss, loss_dict = self.compute_loss(x, h, node_mask, edge_mask, context, t0_always=False)
        else:
            # in the estimator, costs two forward passes.(estimating mode)
    ...
    def sample(self, n_samples, n_nodes, node_mask, edge_mask, context, fix_noise=False):
        """
        Draw samples from the generative model---denosie process+decoder---output=x,h
        1.sample_combined_position_feature_noise随机初始化sample z_t
        2.t--0逐步使用def sample_p_zs_given_zt对zt denosie
        3.sample_p_Xh_given_z0---x,h
        4.check x remove_mean_with_mask?
        5.return x,h
        """
        if fix_noise:
...
class EnHierarchicalVAE(torch.nn.Module):
    """
     ...
     encoder--egmn_encoder
     decoder--egmn_decoder
     achive Geometric Autoencoding
     """
    def __init__(
            self,
            encoder: model_mamba.EGMN_encoder_QM9,
            decoder: model_mamba.EGMN_decoder_QM9,
    ...
    def compute_reconstruction_error(self, xh_rec, xh):
        ...
        L_recon=-Eq(z_x,z_h|x,h)p(x,h|z_x,z_h)
        """
    ...
    def compute_loss(self, x, h, node_mask, edge_mask, context):
        ...
        # loss--as paper equation (6) L_AE=L_recon+L_re
        assert len(loss.shape) == 1, f'{loss.shape} has more than only batch dim.'
...
class EnLatentDiffusion(EnVariationalDiffusion):
    """
      ...
      继承EnvariantionalDiffusion的function
    """
    ...
    def forward(self, x, h, node_mask=None, edge_mask=None, context=None):
        """
        Computes the loss (type l2 or NLL) if training. And if eval then always computes NLL.
        as generative model ,we always care about the loss
        about GeoLDM_loss:1.x,h--encoder--z_x_mu,sigma,z_h_mu,sigma
                          2.get z_xh_mean+sigma--VAE sample from mean & sigma--get z_xh
                          3.if train VAE ,compute loss_VAE(from the process of decoding rescontruction)---compute_reconstruction_error
                          4.get z_x,z_h from z_xh,as input to EnVariantionalDiffusion--compute_loss
                          5.compute when z0时刻 neg_log_constants
                          6.GeoLDM_loss=loss_recon(VAE)+loss_ld(SE3-EGMN)+neg_log_constants
        """
    ...
    def instantiate_first_stage(self, vae: EnHierarchicalVAE):
        ...
        if not self.trainable_ae:
            #VAE model不可training ,不需梯度
            self.vae = vae.eval()
            self.vae.train = disabled_train
            for param in self.vae.parameters():
                param.requires_grad = False
        else:
            #VAE model 可train，需梯度
            self.vae = vae.train()
            for param in self.vae.parameters():
