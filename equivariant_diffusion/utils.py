...
#实现指数移动平均(Exponential Moving Average,EMA),一种平滑技术，用于对模型参数进行动态更新，使其在training中保持稳定，避免震荡
class EMA():
    def __init__(self, beta):
        super().__init__()
        self.beta = beta #beta是平滑因子，取值范围在[0,1) 越接近1，更新越慢；‘’0，‘’越快，控制EMA平滑程度
   #遍历 current_model + ma_modelde 的 parameter
   #对每个parameter 分别调用update_averge进行平滑更新
...
#在用mask的情况下去除mean，仅仅operate practical node data
#通过mask排除填充部分的影响，确保计算的mean准确
...
#数据中心化(center_gravity_zero)--mean=0,使计算更加稳定。像图结构数据重心对齐是自然约束条件
#计算输入张量 x 在一个 中心为零的高斯分布 上的对数似然值（log-likelihood）
...
# Standard Gaussian distribution的对数似然计算和采样
