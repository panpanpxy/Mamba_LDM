...
#实例化用于处理data QM9的EGNN模型
#in_node_nf 是指输入节点的特征数 n_dims 坐标维度，3
#forward的process：1.特征分解（x,h分别提取出来)
# 2.context(conditional_time?context_feature?)
# 3.message passing(update x,h)
# 4.result process
class EGNN_dynamics_QM9(nn.Module):
    def __init__(self, in_node_nf, context_node_nf,
        ...
        self.context_node_nf = context_node_nf #上下文(像分子整体的条件信息 例如温度、压力、时间等)节点特征的维度，在node_features中引入一种conditional features的维度大小
        #初始化egnn时，被添加到in_node_nf的总维度中
        self.device = device
        self.n_dims = n_dims
    ...
    def _forward(self, t, xh, node_mask, edge_mask, context):
        ...
        #egnn or gnn来更新h,x(message passing) vel计算速度矢量，即位置更新的变化量
        ...
        if self.condition_time:
            ...
            h_final = h_final[:, :-1]
        # 对张量vel（代表特征信息）进行reshape，view方法不改变数据本身，只改变张量的排列方式
        # 将数据转换为更结构化的形式，组织成模型易于理解的形式--便于对每个node进行特征维度的操作
        #view的parameters中-1是指自动计算维度的大小，使得新形状与原始数据大小一致
        vel = vel.view(bs, n_nodes, -1)
        #detect vel中是否含有NAN(not a number)value，如果有，打印出warning并将vel reset为全0 tensor
        #torhc.isnan(vel)--return True/False tensor,与vel的形状相同，其中每个位置的value表示对应位置是否未NAN
        #torhc.any()--如果vel张量中任何一个值为NAN，返回True;否则return False
...
#在autocoding中parametering用到EGNN
class EGNN_encoder_QM9(nn.Module):
    def __init__(self, in_node_nf, context_node_nf, out_node_nf,
        ...
        '''
        :param in_node_nf: Number of invariant features for input nodes.'''#输入节点的特征量
        super().__init__()
        #include_charges是一个布尔参数，用于指示模型是否要将原子的电荷特性纳入node特征中
        include_charges = int(include_charges)
        #原子种类数量（node的特征数量中去除原子的电荷特性）
