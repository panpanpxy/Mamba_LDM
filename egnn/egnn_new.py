...
import torch
import math
#不变(invariant)操作--update h(节点特征)、edge_feature 边特征，message passing
...
class GCL(nn.Module):
    ...
    output_nf--输出节点特征维度
    hidden_nf--隐藏层特征维度
    edges_in_d--输入边特征维度(eg.化学键类型)
    nodes_att_dim--节点注意力机制维度(if incorporate attention)
    '''
    def __init__(self, input_nf, output_nf, hidden_nf, normalization_factor, aggregation_method,
        ...
        self.edge_mlp = nn.Sequential(
            ...
            act_fn)
        #节点mlp层
        # in_dim=hidden_nf(前一层EGNN更新后的特征，提供了节点从其领域聚合得到的更丰富的上下文信息)+input_nf+nodes_att_dim(表示原子的重要程度)--比起只是输入input_nf,更全面捕获节点的全局和局部信息，提升model性能
   ...
   #更新边特征
    def edge_model(self, source, target, edge_attr, edge_mask):
        ...
        if self.attention:
            ...
            out = mij * att_val #等效于mi = scatter_sum(mij * eij, dst, dim=0, dim_size=h.shape[0])
        else:
            out = mij
...
class EquivariantUpdate(nn.Module):
   ...
   #更新coordinate 符合SE3-euqivariant的特性，从而EquivariantUpdate层符合SE3-equivariant
    #coord_diff 表示每条边上两个节点坐标的差值（归一化后） 控制着EGNN中坐标更新的幅度和方向，作为局部更新一部分，确保stability+equivariance
...
#这个类是将距离，通过正/余弦映射到高纬度
#基于正弦和余弦的嵌入转换通过将输入数据映射到一个高维的频率空间中，增强model的特征表示能力和非线性能力，使得模型能够更好地理解和处理复杂的输入数据
